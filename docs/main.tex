...
Download the darwin arm64 zip file from the releases page, and place the download in your Applications folder. If you are running OS<15, try opening the app and following system prompts to bypass the security settings that prevent running gravitron as an un-signed application. If that is not an option (ie the system prompt just says the app is bad and should be deleted), open a terminal and run:

\texttt{xattr -d com.apple.quarantine /path/to/gravitron.app}

\noindent where, if you put the app in the suggested location, the path should be \texttt{/Applications/gravitron.app}. You may need to run this command specifically from an administrator account.

There are also options to globally disable the security settings for unsigned apps in OS 15+. We don't recommend that, but if you are interested you can find information by searching the apple forums and/or stackoverflow.
...
\item Enter the tie metadata
    ...
    \item If you are using a ship-specific file format for the meter data (see Section~\ref{file-types}), toggle the switch for ``Ship's read function'' to ON.
...
\subsubsection{Which type of DGS file should I be using?}
\label{file-types}
The laptop attached to the gravimeter, running the DGS software, will be writing two types of files: a `raw' file that contains uncalibrated gravity measurements (AD counts) and a `processed' file that has calibrated gravity values. For a tie, you will always want the `processed' file. The `processed' file format as written directly from the DGS software \textit{should} be the same for all ships. 

\textbf{Files that you get from the ship's shared data drive may have some additional formatting.} For example, some ships will prepend a timestamp from one of their nav systems to each data string. For those files, \texttt{gravitron} tries to provide ship-specific functions to read the meter files. To use these functions, the ``Ship's read function'' toggle switch should be set to ON. However, not all ships are supported, and ship-specific formats may change. If you are unable to read a DGS file because the ship's format is unsupported, you can try pulling the processed file directly from the DGS laptop and reading it with the ``Ship's read function'' toggle switch set to OFF (to use the generic DGS read function).
...
First, make sure that you are using the `laptop' data files rather than the raw files, as those are what gravitron is designed to read. Next, look at Section~\ref{file-types} and make sure you are using the correct read function (ship-specific or not). If that still doesn't work, contact PFPE with a sample of the unreadable file. It may be that gravitron doesn't yet know how to read files from your ship, as there are variations in gravimeter file format between vessels/instruments. Note that you can take and save all of the measurements necessary for a gravity tie and wait until later to compute the bias if there are issues with reading gravimeter files.
